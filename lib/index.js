 ...
 *
 * The configured reasoning effort is only injected when the target model
 * actually supports it: a stale effort (e.g. a value left over from a model
 * that supported it, after the model was switched to one that does not) would
 * otherwise be rejected by the LLM layer on the next request. Model capability
 * lookups are cached per provider/model; the cache is cleared on every
 * settings change so a reconfigured model picks up fresh capabilities.
 */
function installChildOverride(agent, route, resolveEfforts) {
  return agent.ctx.on('agent/request', async (_payload, next) => {
    ...
    if (reasoningEffort !== '') {
      ...
      if (efforts === undefined || efforts.has(reasoningEffort)) {
...
export function apply(ctx) {
  ...
  /** Model capability cache: `provider\u0000model` -> Set<effort> | null | undefined. */
  ...
  const resolveEfforts = async (provider, model) => {
    ...
    try {
      ...
      efforts = info.reasoning === undefined ? new Set() : new Set(info.reasoning.efforts.map((e) => e.id));
    } catch {
      efforts = undefined;
    }
  ...
  // the new values (handled in agent/created). Either way, settings changes
  // invalidate the model-capability cache.
  ...
  ctx.on('agent/created', ({ agent }) => {
    ...
    const dispose = installChildOverride(agent, route, resolveEfforts);
    agent.ctx.effect(() => {
      return () => {
        dispose();
